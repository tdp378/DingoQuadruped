"""Drive Dingo in Gazebo using Twist teleop (``/cmd_vel``) via a Joy bridge.
...
This launch runs **only** ``cmd_vel_to_joy_bridge`` + ``dingo_driver`` (sim, no IMU).
**Do not** also run ``dingo.launch.py`` with keyboard or ``joy_node`` — one ``/joy`` publisher.

Example
-------
Terminal 1: ``ros2 launch dingo dingo_gazebo_sim.launch.py``

Terminal 2::

    source /opt/ros/jazzy/setup.bash
    source ~/DingoQuadruped/dingo_ws/install/setup.bash
    ros2 launch dingo twist_teleop_dingo_sim.launch.py
